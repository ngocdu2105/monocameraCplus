...
RCNN::RCNN(const std::string& pathModel)
{
    Timer t;
    Ort::Env env(ORT_LOGGING_LEVEL_WARNING, "ONNXExample");
    Ort::SessionOptions sOpts;
    // session.reset(new Ort::Session(env, pathModel.c_str(), Ort::SessionOptions()));
    session= Ort::Session(env, pathModel.c_str(), Ort::SessionOptions());
    std::cerr<<"Load success model RCNN ";
    t.stop();
}

cv::Mat RCNN::maskbelow() const
{
    return MatBelow;
}
cv::Mat RCNN::masktop() const
{
    return MatTop;
}
std::vector<float> RCNN::PreprocessImg(const cv::Mat& img)
{
...
void RCNN::run(const cv::Mat& img)
{
    Timer t;
    assert(!img.empty() && "RCNN Input image is empty");
    ...
    std::cerr<<"RCNN model predicts crop image ";
